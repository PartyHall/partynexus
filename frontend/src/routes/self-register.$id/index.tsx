    ...
    <div className='flex flex-col w-1/2 gap-2 m-auto mt-5'>
      {
        (!token || !tokenUser)
        && <>
          <ButtonLink to='/self-register/$id/register' params={{ id }}>{t('register.register')}</ButtonLink>
          <ButtonLink to='/login' search={{ redirect: `/self-register/${id}` }}>{t('login.title')}</ButtonLink>
        </>
      }

      {
        (token && tokenUser)
        && <Button onClick={join}>
          <Translate mapping="register.self.join_as" values={{ username: tokenUser.username }} />
        </Button>
      }
    </div>
  </>
