...
import { checkForgottenPasswordValidity, setNewPassword } from '@/api/users/forgotten_password';
    ...
    if (error instanceof HttpError && error.status === 404) {
      return <div className='flex h-full w-full items-center justify-center'>
        <Card className='w-full sm:w-110 gap-4 flex flex-col text-justify'>
          <h1 className='text-center text-2xl font-bold'>{t('forgotten_password.title')}</h1>
  ...
  return <div className='flex h-full w-full items-center justify-center'>
    <Card className='w-full sm:w-110 gap-4 flex flex-col text-justify'>
      <h1 className='text-center text-2xl font-bold'>{t('forgotten_password.title')}</h1>
      {
        !passwordUpdated &&
        <>
          <p>{t('forgotten_password.desc', { username: getUserName(data.user) })}</p>
          ...
          <form className='flex flex-col gap-2' onSubmit={handleSubmit(onSubmit)}>
            <PasswordInput
              ...
              required
              {...register('newPassword', { required: t('generic.required') })}
            />
      ...
      }
      {
        ...
        && <div className='flex flex-col items-center gap-4'>
          ...
          <ButtonLink to='/login'>{t('forgotten_password.goto_login')}</ButtonLink>
        </div>
      }
    </Card>
  </div>;
