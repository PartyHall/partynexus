...
function RouteComponent() {
  ...
  useTranslatedTitle("admin.backdrop_albums.title_edit", "admin.title", {
    name: album.title,
  });
    ...
    <div className="flex flex-col items-center justify-center w-full mx-auto">
      <Card className="m-auto mt-4 w-full sm:w-150">
        <Title className="text-center" noMargin>
          {t("admin.backdrop_albums.title_edit", { name: album.title })}
        </Title>

        <BackdropAlbumForm
          album={album}
          onSuccess={() =>
            enqueueSnackbar(t("generic.edit_saved"), { variant: "success" })
          }
        />
        ...
        <div className="flex items-center justify-center mt-4">
          ...
          >
            <IconTrash size={18} />
            {t("admin.backdrop_albums.delete_album")}
          </ConfirmButton>
        </div>
      </Card>
      ...
      <Card className="m-auto mt-4 w-full sm:w-150">
        <Title level={2} noMargin>
          {t("admin.backdrop_albums.backdrops.title")}
        </Title>
        ...
        {album.backdrops.length === 0 && (
          <p className="text-center text-primary-100 mt-4">
            {t("admin.backdrop_albums.no_backdrops")}
          </p>
        )}
      </Card>
    </div>
