...
function RouteComponent() {
  ...
  return <InfiniteScroll
    ...
    bottomButtons={[
      <ButtonLink to='/admin/backdrop-albums/new'><IconPlus size={18} />{t('generic.create')}</ButtonLink>
    ]}
    renderItem={item => <CardLink to='/admin/backdrop-albums/$id' params={{ id: '' + item.id }} noGlow>
      <h3 className='text-lg'>{item.title}</h3>
      <p className='text-sm text-gray-500'>{t('admin.backdrop_albums.album_version', {version: item.version})}</p>
