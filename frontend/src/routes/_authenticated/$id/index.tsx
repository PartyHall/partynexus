...
function RouteComponent() {
  ...
  return (
    <div className="flex-1">
      ...
      {event.location && (
        <KeyVal label="events.located_at">{event.location}</KeyVal>
      )}
      {
        <KeyVal label="events.made_by">
          {event.author?.length ? (
            ...
            <Username user={event.owner} noStyle />
          )}
        </KeyVal>
      }
      ...
      <div className="flex flex-col gap-2 mt-4">
        {tokenUser?.id === event.owner?.id && (
          <ButtonLink to="/$id/edit" params={{ id: event.id }}>
            <IconEdit size={18} /> {t("generic.edit")}
          </ButtonLink>
        )}
        {event.export && <ExportDownloadButton event={event} />}
      </div>
    </div>
  );
}
