...
function RouteComponent() {
  ...
  const addParticipant = async () => {
    ...
    try {
      ...
      enqueueSnackbar(
        t("events.editor.participants.add_success", {
  ...
  return (
    <div className="flex flex-col gap-4">
      <Title noMargin className="text-center">
      ...
      {(isGranted("ROLE_ADMIN") || tokenUser?.id === event.owner.id) && (
        <div className="flex flex-row items-center gap-2 mb-4">
          ...
          <AsyncSelect
            ...
            menuPosition="fixed"
            unstyled
            loadOptions={async (val: string) =>
              (await getUsers({ search: val })).member.map((u) => {
            ...
            classNames={{
              ...
              option: ({ isSelected, isFocused }) =>
                `cursor-pointer rounded-sm p-1 ${isSelected || isFocused ? "bg-synthbg-400" : ""}`,
      ...
      <Modal
        ...
        description={t("events.editor.participants.add_disclaimer", {
          username: userToAdd?.label,
        })}
      >
        <div className="flex flex-row gap-2 align-center justify-end w-full">
          <Button onClick={() => setUserToAdd(null)} variant="secondary">
            {t("generic.cancel")}
          </Button>
          <Button onClick={addParticipant} disabled={isAdding}>
            {t("generic.yes")}
          </Button>
        </div>
      </Modal>
