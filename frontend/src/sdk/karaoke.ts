...
export default class Karaoke {
    ...
    async searchExternal(
        ...
        artist: string,
        title: string
    ): Promise<Collection<PnExternalSong> | null> {
        const resp = await this.sdk.get(
            `/api/external/${provider.toLowerCase()}/${artist}/${title}`
        );
        const data = await resp.json();
    ...
    async requestMusic(title: string, artist: string) {
        const resp = await this.sdk.post(`/api/song_requests`, {
            title,
            artist,
        });
        const data = await resp.json();
