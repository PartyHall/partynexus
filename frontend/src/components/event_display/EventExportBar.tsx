...
export default function EventExportBar({ pnExport }: Props) {
    ...
    return (
        <Flex vertical>
            ...
            <KeyVal label={t('event.export.started_at')}>
                {pnExport.startedAt.format('HH:mm:ss - DD / MM / YYYY')}
            </KeyVal>
            {pnExport.endedAt && (
                <KeyVal label={t('event.export.ended_at')}>
                    {pnExport.endedAt.format('HH:mm:ss - DD / MM / YYYY')}
                </KeyVal>
            )}
            <KeyVal label={t('event.export.progress')}>
                {pnExport.progress}
            </KeyVal>
