...
export default function EventActionBar({
    ...
    return (
        <Flex align="center" justify="space-around" style={{ marginTop: 8 }}>
            {displayOwnerStuff && !event.over && (
                <Popconfirm
            ...
            {displayOwnerStuff && event.over && (
                <Button icon={<IconFileArrowRight />} onClick={onConclude}>
                    {t('event.force_export')}
                </Button>
            )}
            {event.export && (
