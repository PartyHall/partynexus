...
export default function SongFileUploader({type, song, extensions}: Props) {
  ...
  return <Card>
    <Flex vertical gap={8}>
      <Typography.Title level={3} style={{margin: 0}}>{type}: {(type === 'lyrics' && <>{song.cdgFileUploaded ? <IconCheck size={20}/> : <IconX size={20}/>}</>)}</Typography.Title>
      ...
      <Flex align="center" justify="space-around">
        ...
        }
        {
          ...
          && (song.format?.toLowerCase() === 'video' || song.format?.toLowerCase() === 'transparent_video')
          && <video style={{width: '50%'}} controls src={song.instrumentalUrl} />
        }
        {
          type === 'instrumental'
          && song.instrumentalUrl
