...
function DisplayExternalSong({song, onClick}: {song: PnExternalSong, onClick: () => void}) {
    ...
    return <Card onClick={onClick} className="song">
        <Flex gap={8} align="center">
            { song.cover && <img src={song.cover} /> }
            <Flex vertical flex="1">
                <Typography.Text>{t('karaoke.editor.search_external.track')}: {song.title}</Typography.Text>
                <Typography.Text>{t('karaoke.editor.search_external.artist')}: {song.artist}</Typography.Text>
                <Typography.Text>{t('karaoke.editor.search_external.id')}: {song.id}</Typography.Text>
            </Flex>
        </Flex>
    </Card>
}
...
export default function ExternalSongSearch({ shown, close, setId, provider }: Props) {
    ...
    >
        <Flex vertical gap={8}>
            <Flex vertical>
                <Typography.Text>{t('karaoke.editor.search_external.artist')}:</Typography.Text>
                <Input value={artist} onChange={x => setArtist(x.target.value)} />
                <Typography.Text>{t('karaoke.editor.search_external.track')}:</Typography.Text>
                <Input value={track} onChange={x => setTrack(x.target.value)} />
            </Flex>

            <Flex vertical gap={8} className="externalSongSearchList">
                {
                    songs
                    && songs.items.map(x => <DisplayExternalSong
                        key={x.id}
                        song={x}
                        onClick={() => {
                            setId(x.id);
