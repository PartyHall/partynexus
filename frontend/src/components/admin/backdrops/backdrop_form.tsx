  ...
  return (
    ...
    >
...

      {
        ...
        !backdrop?.id && (
          <label className="flex flex-col w-full gap-0.5̀">
            <span>
              {t("admin.backdrop_albums.backdrops.image")}:
...
export function BackdropFormModal({
  open,
  onClose,
  onSuccess,
  albumIri,
  ...
  return (
    <Modal
      ...
      title={t(
        "admin.backdrop_albums.backdrops." +
          (backdrop ? "edit_title" : "new_title"),
        { title: backdrop?.title },
      )}
      description={t(
        "admin.backdrop_albums.backdrops." +
