...
export default function UserAuthenticationLogs({ user }: Props) {
    ...
    return <div className="overflow-auto">
        ...
        {isError && <p>{t('generic.error.generic')}: {error instanceof HttpError ? t(error.message) : String(error)}</p>}
                ...
                <ul>
                    {data.member.map((log: UserAuthLog) => (
                        <li className='bg-synthbg-900 rounded-md p-2 m-1' key={log.id}>
                            <p>{t('generic.date')}: {dayjs(log.authedAt).format('L - LT')}</p>
                            <p>{t('generic.ip')}: {log.ip}</p>
                        </li>
                    ))}
                </ul>
            )
        ...
        {
            data && data.member.length === 0
            && <p className="text-center text-primary-100">{t('admin.users.auth_log.empty')}</p>
        }
    </div>;
}