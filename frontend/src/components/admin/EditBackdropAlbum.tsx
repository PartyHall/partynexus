...
const Editor = ({ album, onUpserted }: EditorProps) => {
    ...
    return <Form onFinish={handleSubmit(submit)}>
        <Flex vertical style={{ padding: '0 1.5em' }}>
            ...
            >
                <InputNumber disabled={formState.isSubmitting} min={1} style={{ width: '100%' }} />
            </FormItem>
        </Flex>
...
export default function EditBackdropAlbum({ albumId, onUpserted }: Props) {
    ...
    const [loadingAlbum, setLoadingAlbum] = useState<boolean>(albumId !== undefined);
    ...
    return <Loader loading={loadingAlbum}>
        {
            album && <Editor
                album={album ?? null}
