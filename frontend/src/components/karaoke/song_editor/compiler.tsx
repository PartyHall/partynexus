...
export default function SongEditorCompiler({ song, doInvalidate }: Props) {
  ...
  return (
    <div className="w-full sm:w-120 flex flex-col gap-4">
      <Title level={2} className="text-center" noMargin>
        {t("karaoke.compile.title")}
      </Title>

      <p>{t("karaoke.compile.text1")}</p>
      <p>{t("karaoke.compile.text2")}</p>
      <p>{t("karaoke.compile.text3")}</p>

      <div className="flex justify-center">
        <Button onClick={doCompile} disabled={compiling}>
          {t(
            `karaoke.compile.request_${song?.ready ? "decompile" : "compile"}`,
          )}
        </Button>
      </div>
    </div>
