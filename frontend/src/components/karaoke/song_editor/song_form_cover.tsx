...
export default function SongFormCover<T extends FieldValues>({
  song,
  name,
  disabled,
  control,
  ...
  const [songUrl, setSongUrl] = useState<string>(
    song?.coverUrl || placeholderCoverUrl,
  );
        ...
        <Tooltip content={t("karaoke.editor.upload_cover")}>
          <UploadButton
            ...
            onChange={(file) => {
              ...
              setSongUrl(
                file ? URL.createObjectURL(file) : placeholderCoverUrl,
              );
            }}
          >
            <IconUpload size={18} />
      ...
      {selectedFileName && (
        <div className="absolute w-full mt-2 text-xs text-primary text-center text-primary-glow animate-pulse top-0 flex flex-col bg-primary-900/90">
          ...
          <span className="ml-2 opacity-80">
            {t("karaoke.editor.not_uploaded_yet")}
          </span>
        </div>
      )}
    </div>
