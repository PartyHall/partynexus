...
export default function MediaList({
  ...
  return (
    <Card className="flex flex-col gap-2 items-center">
      <Title level={3} className="mb-2" noMargin>
        {t("karaoke.editor." + type.toLowerCase())}
      </Title>
      ...
      {hasAnUpload && type === "CDG" && (
        <span>{t("karaoke.editor.cdg_uploaded")}</span>
      )}

      <UploadMediaButton
        song={song}
        type={type}
        onUploadComplete={onMediaUploaded}
      />
    </Card>
  );
}
