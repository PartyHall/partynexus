...
export default function Input({
  ...
  if (props.type === "hidden") {
    return (
      <input
        className={`block w-full ${icon ? "!pl-10" : ""} ${action ? "!pr-8" : ""} disabled:bg-synthbg-600! disabled:text-synthfg-400!`}
        {...props}
      />
    );
  }
    ...
    <Controller
      ...
      render={({ field }) => (
        <Input
          ...
          label={label}
          value={
            field.value ? dayjs(field.value).format("YYYY-MM-DDTHH:mm") : ""
          }
          error={error}
