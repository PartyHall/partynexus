...
export default function ApplianceEditor({ appliance: initialAppliance }: Props) {
    const { t } = useTranslation();
    const { api } = useAuth();
    const [appliance, setAppliance] = useState<PnAppliance | null>(initialAppliance);
    ...
    return <Flex vertical align="center">
        <Typography.Title className="blue-glow">{t('my_account.appliance_editor.title')}</Typography.Title>
        ...
        >
            {
                appliance?.id
                && <Typography.Text style={{fontSize: '1.1em'}}>ID: {appliance.id}</Typography.Text>
            }
...

            {
                appliance?.id &&
                <Flex vertical gap={4}>
                    <Typography.Text style={{fontSize: '1.1em'}}>Hardware ID:</Typography.Text>
                    <CopyField text={appliance.hardwareId} />
                    <Typography.Text style={{fontSize: '1.1em'}}>Token:</Typography.Text>
