...
export default function ApplianceCard({
  ...
  const deleteAppliance = async () => {
    setIsDeleting(true);
    try {
      await customFetch(`/api/appliances/${appliance.id}`, {
        method: "DELETE",
      });
    } catch (error) {
  ...
  return (
    <div className="w-full bg-synthbg-700 p-3 rounded-lg flex flex-row justify-between items-center">
      ...
      <div className="flex flex-row items-center gap-2">
        ...
        <Modal
          ...
          description={t("account.my_appliances.delete_confirm", {
            name: appliance.name,
          })}
          actions={
            <>
