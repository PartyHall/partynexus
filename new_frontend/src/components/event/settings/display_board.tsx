...
export default function SettingsDisplayBoard() {
    ...
    return <div className='flex flex-col gap-2'>
        <Title level={2} noMargin>{t('events.settings.display_board.title')}</Title>
        <p>{t('events.settings.display_board.desc')}</p>
        {
            (event.displayBoardKey || key)
            && <div className="mt-2 flex items-center justify-center">
                <a href={event.displayBoardKey?.url ?? key!} target="_blank" rel="noreferrer">
...

        {
            ...
            && <>
                ...
                <Button disabled={creating} onClick={createDisplayBoard}>{t('events.settings.display_board.generate')}</Button>
            </>
        }
    </div>;
}