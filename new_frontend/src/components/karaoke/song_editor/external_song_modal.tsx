...
export default function ExternalSongModal({ service, defaultArtist, defaultTitle, open, onClose }: Props) {
    ...
    useEffect(() => {
        (async () => {
            ...
            try {
                const resp = await customFetch(`/api/external/${service.toLowerCase()}/${debouncedArtist}/${debouncedTitle}`);
    ...
    >
        <Input label={t('karaoke.song_title')} value={title} onChange={x => setTitle(x.target.value)} />
        <Input label={t('karaoke.song_artist')} value={artist} onChange={x => setArtist(x.target.value)} />

        <div className="overflow-y-auto">
            {
                results.map(x => <ExternalSongCard
                    key={x.id}
                    song={x}
                    onSelect={() => onClose(x)}
                />)
